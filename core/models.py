...
class Emprestimo(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk:
            ...
            antes = Emprestimo.objects.get(pk=self.pk)

            if antes.livro != self.livro:

                antes.livro.quantidade += 1
                antes.livro.save()
    ...
    def delete(self, *args, **kwargs):
        """
        Regras simples ao excluir:
        - Se estava ABERTO → o livro ainda está com o aluno → devolver 1 ao estoque
        - Se estava DEVOLVIDO → estoque já está certo, não mexe
        """
        if self.status == "ABERTO":
            self.livro.quantidade += 1
            self.livro.save()
